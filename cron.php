...
if (php_sapi_name() == "cli") {
    ...
    foreach ($json_a as $key => $value) {
        ...
        $val_domain = validate_domains($domains_b);
        if (count($val_domain['errors']) >= 1) {
            ...
            foreach ($errors as $error_value) {
                ...
                if (strpos($error_value, 'Domain has expired certificate in chain') === false) {
                    send_error_mail($domain, $port, $email, $errors);
                }
            }

            $json_a[$key]['errors'] += 1;
            $check_json = json_encode($json_a, JSON_PRETTY_PRINT);
        ...
        $raw_chain = get_raw_chain($domain, $port);
        $counter = 0;
        foreach ($raw_chain['chain'] as $chain_key => $chain_value) {
            ...
            cert_expiry_emails($domain, $port, $email, $cert_expiry, $chain_value);
    ...
    foreach ($tmp_pre_json_a as $pre_key => $pre_value) {
        ...
        if ($pre_add_diff > "604800") {
            ...
            $tmp_pre_json = json_encode($tmp_pre_json_a, JSON_PRETTY_PRINT);
